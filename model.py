...
def main():
    model, tokenizer = load_base_model_and_tokenizer(model_name='unsloth/Qwen2.5-0.5B-Instruct-bnb-4bit', max_seq_length=256)
...
def count_total_parameters(model):
    ...
    count_of_parameters = sum(p.numel() for p in model.parameters())
...
def ensure_pad_token(tokenizer:PreTrainedTokenizerBase) -> PreTrainedTokenizerBase:
    ...
    if tokenizer.pad_token is None:
        if tokenizer.eos_token is None:
            raise ValueError("Tokenizer has neither a pad_token nor an eos_token, "
                              "so a pad token cannot be assigned.")
...
def attach_lora_adapters(model, r=8, lora_alpha=16, target_modules=None):
    ...
    model = FastLanguageModel.get_peft_model(model, r=r, target_modules = target_modules, lora_alpha=lora_alpha, lora_dropout=0, bias="none", use_gradient_checkpointing= "unsloth", random_state = 3407)
    return model
...
def count_trainable_parameters(model):
    ...
    count_trainable_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
...
def build_instruction_examples():
    ...
    instruction_response = [
        ...
        {"instruction": "Write a haiku about the ocean.",
            "response": "Gentle waves whisper\nMoonlight dances on the tide\nNight embraces sea"},
        {"instruction": "What is 12 multiplied by 8?",
            "response": "12 multiplied by 8 is 96."},
        { "instruction": "Give one tip for staying productive.",
            "response": "Break large tasks into smaller steps and focus on one step at a time."},
    ]
...
def format_all_examples(examples):
    ...
    final_list = []
    for i in examples:
        final_list.append(format_instruction_example(i))
    return final_list
...
def build_chat_prompt(tokenizer, instruction):
    ...
    inputs = tokenizer.apply_chat_template(message, add_generation_prompt=True, tokenize=False)
    return inputs
...
def generate_reply(model, tokenizer, prompt, max_new_tokens=32):
    ...
    inputs_1 = {k:v.to(model.device)for k, v in inputs_1.items()}
    outputs = model.generate(**inputs_1, max_new_tokens=max_new_tokens, do_sample=False) # uses greedy decoding(do_smaple=False)
    new_tokens = outputs[0, inputs_1["input_ids"].shape[1]:]
